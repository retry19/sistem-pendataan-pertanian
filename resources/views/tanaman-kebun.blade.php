...
@section('title', 'Tanaman Kebun :: '. config('app.name'))
@section('heading', 'Tanaman Kebun')
...
<div class="card card-outline card-primary" id="card-submit" style="display: none">
  <div class="card-header">
    <h5 class="card-title" id="card-title">Tambah Tanaman Kebun</h5>
  ...
  <form id="form" enctype="multipart/form-data">
    ...
    <div class="card-body">
      ...
      <div class="form-group row">
        <label for="ditambah" class="col-md-3 col-form-label">Tambah Tanaman <small class="text-muted">(pohon)</small></label>
        <div class="col-md-9">
...
<div class="card">
  <div class="card-header">
    <h5 class="card-title">Daftar Tanaman Kebun</h5>
    <div class="card-tools">
      @can('tanaman_kebun_create')
      <button type="button" class="btn btn-tool" id="btn-open-card-add">
        <i class="fas fa-plus"></i>&nbsp; Tambah Tanaman Kebun
      </button>
      @endcan
    </div>
  </div>
    ...
    <table class="table table-bordered table-hover text-sm" id="table">
      <thead>
        <tr class="text-center">
          ...
          <th>Tanaman Akhir <small class="text-muted">(pohon)</small></th>
...
<script>
  ...
  let table = $('#table').DataTable({
    ...
    processing: true,
    serverSide: true,
    ajax: '{{ route('tanaman-kebun.index') }}',
    columns: [
  ...
  document.getElementById('btn-open-card-add').addEventListener('click', function(event) {
    cardTitleFormSubmit.innerHTML = 'Tambah Tanaman Kebun';
    clearAllInput();
  ...
  function onClickEdit(event) {
    ...
    cardTitleFormSubmit.innerHTML = 'Edit Tanaman Kebun';
    clearAllInput();
    document.getElementById('id').value = id;
    document.getElementById('_method').value = 'put';

    fetch(`{{ route('tanaman-kebun.index') }}/${id}/edit`)
      .then(res => res.json())
      .then(data => {
        if (data.status) {
          let _data = data.data;
  ...
  function onClickDelete(event) {
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        fetch(`{{ route('tanaman-kebun.index') }}/${id}`, {
          headers: headersJSON,
          method: 'post',
          body: JSON.stringify({
            _method: 'delete'
  ...
  function handleOnSubmit(event) {
    ...
    let url = method == 'put' 
      ? `{{ route('tanaman-kebun.store') }}/${id}`
      : '{{ route('tanaman-kebun.store') }}'
